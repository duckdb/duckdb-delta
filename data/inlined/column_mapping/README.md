...
**No data files**, except where the row says so. The tables start empty, so every parquet file the
test inspects is one it just wrote — which is the point: the assertions are about what our writer
emits.
